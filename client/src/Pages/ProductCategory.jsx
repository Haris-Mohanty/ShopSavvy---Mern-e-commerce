        ...
        <div className="md:hidden">
          ...
          <div className="fixed bottom-4 right-4 z-50">
            <button
              ...
              className="bg-indigo-500 text-white py-2 px-4 rounded-full shadow-md hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-400 transition-colors duration-300"
            >
              Filters
            </button>
          </div>
          ...
          {/* Mobile Filter Panel */}
          {showMobileFilter && (
            <div className="fixed inset-0 z-40 bg-gray-900 bg-opacity-50 flex justify-end">
              <div className="bg-white w-3/4 max-w-md h-full overflow-y-auto shadow-lg">
                <div className="px-6 py-4">
                  <div className="flex justify-between items-center">
                    ...
                    <button
                      onClick={toggleMobileFilters}
                      className="text-gray-700 hover:text-indigo-500 focus:outline-none focus:text-indigo-500 transition-colors duration-300"
                    >
                      Close
                    </button>
                  </div>
                  ...
                  {/* Price Filter */}
                  <div className="mb-2">
                    <h3 className="text-lg font-semibold my-1 text-indigo-500">
                      Price
                    </h3>
                    <div className="flex flex-col space-y-1">
                      <label className="inline-flex items-center cursor-pointer">
          ...
          <div className="grid grid-cols-2 gap-2 mb-2 px-3">
            {products.length > 0 ? (
              products.map((product) => (
                <div
                  key={product._id}
                  className="relative flex flex-col text-gray-700 bg-white shadow-md bg-clip-border rounded-xl w-[10.5rem]"
                >
                  <div className="relative mx-4 mt-4 overflow-hidden text-gray-700 bg-white bg-clip-border rounded-xl h-32">
                    <Link
                      className="mx-3 flex flex-col h-32 overflow-hidden rounded-xl"
                      to={`/product-details/${product._id}`}
                    >
                      <div className="overflow-y-scroll h-32 scrollbar-none">
                        {product.productImage.map((img, index) => (
                          <img
                            key={index}
                            className="object-scale-down h-full w-full"
                  ...
                  <div className="p-2">
                    <div className="mb-1">
                      <Link
                        to={`/product-details/${product._id}`}
                        className="font-sans text-sm antialiased font-medium leading-relaxed text-blue-gray-900 text-ellipsis line-clamp-1 hover:text-indigo-600 cursor-pointer mb-2"
                      >
                        {product.productName}
                      </Link>
                      <p>
                        ...
                        <span className="text-xs text-indigo-400 line-through px-2">
                          {displayInr(product.price)}
                        </span>
                      </p>
                    </div>
                    <p className="font-sans text-xs antialiased font-normal leading-normal text-gray-700 opacity-75 text-ellipsis line-clamp-3 mt-1">
                      {product.description}
                    </p>
                  </div>
                  <div className="p-3 pt-0">
                    <button
                      type="button"
                      onClick={() => itemsAddToCart(product._id, dispatch)}
                      className="align-middle select-none font-sans font-bold text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none text-xs py-1 px-3 rounded-lg shadow-gray-900/10 hover:shadow-gray-900/20 focus:opacity-[0.85] active:opacity-[0.85] active:shadow-none block w-full bg-blue-gray-900/10 text-blue-gray-900 shadow-none hover:scale-105 hover:shadow-none focus:scale-105 focus:shadow-none active:scale-100"
                    >
                      Add to Cart
                    </button>
                  </div>
